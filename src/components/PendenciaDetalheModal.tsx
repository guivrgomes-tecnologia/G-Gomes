...
export default function PendenciaDetalheModal({ pendenciaId, onClose, onEditar, onChanged }: {
  ...
  async function enviarComentario() {
    ...
    const autorRespondido = respondendoComentario
      ? (respondendoComentario.autor_id === user?.id ? 'Você' : (respondendoComentario.autor as Profile | undefined)?.nome ?? null)
      : null
                ...
                return (
                  <div key={c.id} className={`flex items-start gap-1 group ${souEu ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[80%] rounded-xl px-3 py-2 ${souEu ? 'bg-brand-600 text-white' : 'bg-gray-100 text-gray-800'}`}>
                      ...
                      {c.resposta_a_texto && (
                        <div className={`flex flex-col gap-0.5 rounded-lg px-2 py-1 mb-1.5 border-l-2 ${souEu ? 'bg-white/15 border-white/50' : 'bg-white border-brand-400'}`}>
                          <p className={`text-[10px] font-semibold ${souEu ? 'text-white/80' : 'text-brand-600'}`}>{c.resposta_a_autor}</p>
                          <p className={`text-xs truncate ${souEu ? 'text-white/80' : 'text-gray-500'}`}>{c.resposta_a_texto}</p>
                        </div>
                      )}
                      {c.tarefa_texto && (
              ...
              <div className="flex items-center gap-2 bg-gray-50 border-l-2 border-brand-500 rounded-lg px-2.5 py-1.5 mb-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[10px] font-semibold text-brand-600">
                    Respondendo {respondendoComentario.autor_id === user?.id ? 'a você' : (respondendoComentario.autor as Profile | undefined)?.nome ?? ''}
                  </p>
                  <p className="text-xs truncate text-gray-600">{respondendoComentario.mensagem || '📷 Imagem'}</p>
                </div>
                <button onClick={() => setRespondendoComentario(null)} className="text-gray-400 hover:text-gray-600 shrink-0"><X size={14} /></button>
              </div>
            )}
            {respondendoTarefa && (
              <div className="flex items-center gap-2 bg-gray-50 border-l-2 border-brand-500 rounded-lg px-2.5 py-1.5 mb-2">
